...
`, resourceId, wrapupCode1ResourceId),
				Check: resource.ComposeTestCheckFunc(
					validateStringInArray("genesyscloud_outbound_wrapupcodemappings."+resourceId, "default_set", "Contact_UnCallable"),
					validateStringInArray("genesyscloud_outbound_wrapupcodemappings."+resourceId, "default_set", "Number_UnCallable"),
			...
			{
				Config: generateRoutingWrapupcodeResource(wrapupCode1ResourceId, wrapupCode1Name) +
					generateRoutingWrapupcodeResource(wrapupCode2ResourceId, wrapupCode2Name) +
					fmt.Sprintf(`
resource "genesyscloud_outbound_wrapupcodemappings"	"%s" {
...
`, resourceId, wrapupCode1ResourceId, wrapupCode2ResourceId),
				Check: resource.ComposeTestCheckFunc(
					validateStringInArray("genesyscloud_outbound_wrapupcodemappings."+resourceId, "default_set", "Contact_UnCallable"),
					validateStringInArray("genesyscloud_outbound_wrapupcodemappings."+resourceId, "default_set", "Right_Party_Contact"),
					verifyWrapupCodeMappingsMappingValues("genesyscloud_outbound_wrapupcodemappings."+resourceId,
						"genesyscloud_routing_wrapupcode."+wrapupCode1ResourceId, []string{"Contact_UnCallable"}),
					verifyWrapupCodeMappingsMappingValues("genesyscloud_outbound_wrapupcodemappings."+resourceId,
			...
			// Update
			{
				Config: generateRoutingWrapupcodeResource(wrapupCode1ResourceId, wrapupCode1Name) +
					generateRoutingWrapupcodeResource(wrapupCode2ResourceId, wrapupCode2Name) +
					generateRoutingWrapupcodeResource(wrapupCode3ResourceId, wrapupCode3Name) +
					fmt.Sprintf(`
resource "genesyscloud_outbound_wrapupcodemappings"	"%s" {
	...
	return func(state *terraform.State) error {
		...
		for i := 0; i < numValues; i++ {
			if resourceState.Primary.Attributes["mappings."+strconv.Itoa(i)+".wrapup_code_id"] == expectedWrapupCodeId {
				...
				if !listsAreEquivalent(flagsList, expectedFlags) {
