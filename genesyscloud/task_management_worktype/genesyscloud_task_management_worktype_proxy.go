...
type getTaskManagementWorktypeIdByNameFunc func(ctx context.Context, p *taskManagementWorktypeProxy, name string, workbinId string) (id string, retryable bool, err error)
...
type getTaskManagementWorktypeStatusByIdFunc func(ctx context.Context, p *taskManagementWorktypeProxy, worktypeId string, statusId string) (status *platformclientv2.Workitemstatus, responseCode int, err error)
...
func (p *taskManagementWorktypeProxy) getTaskManagementWorktypeIdByName(ctx context.Context, name string, workbinId string) (id string, retryable bool, err error) {
	return p.getTaskManagementWorktypeIdByNameAttr(ctx, p, name, workbinId)
}
...
func createTaskManagementWorktypeFn(ctx context.Context, p *taskManagementWorktypeProxy, taskManagementWorktype *platformclientv2.Worktypecreate) (*platformclientv2.Worktype, error) {
	worktype, _, err := p.taskManagementApi.PostTaskmanagementWorktypes(*taskManagementWorktype)
	if err != nil {
...
func getTaskManagementWorktypeIdByNameFn(ctx context.Context, p *taskManagementWorktypeProxy, name string, workbinId string) (id string, retryable bool, err error) {
	pageSize := 100

	filterType := "String"
	filterOperator := "EQ"
	filterWorkbinIdKey := "defaultWorkbinId"
	filterNameKey := "name"
	filterWorkbinValues := []string{workbinId}
	filterNameValues := []string{name}

	queryReq := &platformclientv2.Worktypequeryrequest{
		PageSize: &pageSize,
		Filters: &[]platformclientv2.Workitemfilter{
			// Filter for the default workbin id
			platformclientv2.Workitemfilter{
				Name:     &filterWorkbinIdKey,
				VarType:  &filterType,
				Operator: &filterOperator,
				Values:   &filterWorkbinValues,
			},
	...
	worktypes, _, err := p.taskManagementApi.PostTaskmanagementWorktypesQuery(*queryReq)
	if err != nil {
		return "", false, fmt.Errorf("failed to get worktype %s from workbin %s: %v", name, workbinId, err)
	}

	if worktypes.Entities == nil || len(*worktypes.Entities) == 0 {
		return "", true, fmt.Errorf("no task management worktype found with name %s from workbin %s", name, workbinId)
	}
