...
func TestAccResourceTfExportDuplicateDataActions(t *testing.T) {
	var (
		exportTestDir = testrunner.GetTestTempPath(".terraform" + uuid.NewString())
	)
	...
	defer func(path string) {
		if err := os.RemoveAll(path); err != nil {
			log.Printf("failed to clean up dir: %s", err.Error())
		}
	}(exportTestDir)

	theconfig := fmt.Sprintf(`
resource "genesyscloud_tf_export" "export1" {
...
locals {
  shared_action_name = "Example Action 2904"
  integration_name   = "Genesys Cloud Data 2904"
}
