...
func fetchDepConsumers(ctx context.Context,
		...
		if data != nil && data.PublishedVersion != nil && data.PublishedVersion.Id != nil {
			...
			if flowTypeExists {
				...
				if dependencies.Entities == nil || len(*dependencies.Entities) == 0 {
					log.Printf("Retrieved dependencies for ID  noresult %v, resourceKey %s, length %s", resources, resourceKey, len(resources))
				...
				if pageCount < 2 {
					...
					log.Printf("Retrieved dependencies for resourceKey %s, resources %v, length %s", resourceKey, resources, len(resources))
					return resources, dependsMap, cyclicDependsList, nil
				}
	...
	log.Printf("Retrieved dependencies for ID %v, resourceKey %s, length %s", resources, resourceKey, len(resources))
	return resources, dependsMap, cyclicDependsList, nil
}
