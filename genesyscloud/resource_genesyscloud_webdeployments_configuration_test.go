...
func TestAccResourceWebDeploymentsConfigurationComplex(t *testing.T) {
	...
	var (
		...
		cobrowseChannelsUpdate = []string{"Webmessaging", "Voice"}
	)

	if !isCoBrowseChannelsFeatureImplemented() {
		cobrowseChannels = nil
		cobrowseChannelsUpdate = nil
	} else {
		t.Log("The cobrowse.channels feature has been implemented. Uncomment the lines to validate these values.")
	}
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					//resource.TestCheckResourceAttr(fullResourceName, "cobrowse.0.channels.#", "1"), // TODO: uncomment these lines when cobrowse.channels has been implemented
