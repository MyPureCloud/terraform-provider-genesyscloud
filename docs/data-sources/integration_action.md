...
# Disambiguate when a static and custom action share the same name under the same integration.
# Use action_type to specify whether you want the built-in ("static") or user-created ("custom") action.
data "genesyscloud_integration_action" "staticActionByType" {
