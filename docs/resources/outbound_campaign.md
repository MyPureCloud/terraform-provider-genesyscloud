...
- `campaign_status` (String) The current status of the Campaign. A Campaign may be turned 'on' or 'off'. Required for updates. When creating, a campaign will be initially created in an "off" state regardless of this value. If the value of campaign_status is set to "on" at time of creation, a subsequent update will occur immediately after creation to turn the campaign on.
