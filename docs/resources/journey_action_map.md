...
- `trigger_with_outcome_probability_conditions` (Block Set) DEPRECATED. Please switch to using quantile contidions instead. Probability conditions for outcomes that must be satisfied to trigger the action map. (see [below for nested schema](#nestedblock--trigger_with_outcome_probability_conditions))
