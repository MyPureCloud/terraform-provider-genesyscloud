...
func TestExportCommand_Markdown(t *testing.T) {
	// Create test data directory with genesyscloud subdirectory to match discovery logic
	testDataDir := "./testdata_markdown"
	genesyscloudDir := filepath.Join(testDataDir, "genesyscloud")
	if err := os.MkdirAll(genesyscloudDir, 0755); err != nil {
		t.Fatalf("Failed to create test data directory: %v", err)
	}
	defer os.RemoveAll(testDataDir)

	// Create a test schema file with annotations (must end with _schema.go)
	testSchemaFile := filepath.Join(genesyscloudDir, "test_resource_schema.go")
	testContent := `package test_package
// @team: Test Team
// @chat: #test-team
// @description: Test resource
...
`
	...
	// Set the global discoverPath variable that export uses
	oldDiscoverPath := discoverPath
	discoverPath = testDataDir
	defer func() { discoverPath = oldDiscoverPath }()
...
`
	...
	// Check error message in either stderr output or error itself
	if !strings.Contains(output, "unsupported format") && !strings.Contains(err.Error(), "unsupported format") {
		t.Errorf("Expected error message about unsupported format, got: %v, output: %s", err, output)
	}
}

func TestValidateCommand(t *testing.T) {
	// Create test data directory
	testDataDir := "./testdata_validate"
	if err := os.MkdirAll(testDataDir, 0755); err != nil {
		t.Fatalf("Failed to create test data directory: %v", err)
	}
	defer os.RemoveAll(testDataDir)
...
`
	...
	validateCmd.SetArgs([]string{"--path", testDataDir})
